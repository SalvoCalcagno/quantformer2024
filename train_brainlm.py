...
def parse():
    ...
    #parser.add_argument('--class-filter', type=int, nargs="+", help="select subset of classes", default=[3, 4, 5, 37, 50])
    
    # Things-EEG
    parser.add_argument('--subject', type=int, nargs="+", help="select given subjects", default=set(range(1, 51)) - {6})
    # IIT
    ...
    parser.add_argument('--noise-magnitude', type=float, default=0.05)
    parser.add_argument('--window-size', type=int, default=160)
    parser.add_argument('--stride', type=int, default=20)
    parser.add_argument('--source-length', type=int, default=96)
    parser.add_argument('--multivariate', action='store_true')
    parser.add_argument('--response-ids', type=str, nargs="+", help="select subset of response files", default=None)
    # Sinusoids
    parser.add_argument('--sequence-length', type=int, default=14)
    #parser.add_argument('--source-length', type=int, default=10)
    parser.add_argument('--num-samples', type=int, default=1000)
    # Allen
    parser.add_argument('--experiment-ids', type=str, nargs="+", help="select subset of response files", default=None)
    parser.add_argument('--quantize', action='store_true')
    parser.add_argument('--quantization-strategy', type=str, default='uniform')
    parser.add_argument('--use-levels', action='store_true')
    parser.add_argument('--num-levels', type=int, default=10)
    parser.add_argument('--windowing-strategy', type=str, default='peaks') # peaks, random, uniform
    parser.add_argument('--select-stimuli', type=str, nargs="+", help="select subset of stimuli", default=None)
    parser.add_argument('--split-type', type=str, default='stimuli')
    parser.add_argument('--use-stimuli-embeddings', action='store_true')
    ...
    parser.add_argument('--trace_format', type=str, default='dff', help='dff or corrected_fluorescence')
    parser.add_argument('--labels_format', type=str, default='outliers')
    parser.add_argument('--threshold', type=float, default=0.1)
    
    # Experiment options
    parser.add_argument('-t', '--tag', default=None)
    parser.add_argument('--project', default='YOUR_PROJECT')
    parser.add_argument('--entity', default='YOUR_ENTITY')
    ...
    # Model options
    parser.add_argument('--model', default='quantformer')
    parser.add_argument('--verbose', action='store_true')

    # Mixed model-specific options
    # EEGNet
    parser.add_argument('--num-channels', type=int, default=63)
    parser.add_argument('--num-classes', type=int, default=1854)
    # SyncNet
    parser.add_argument('--input-size', type=int, default=171)#syncnet 276, #rnn 1
    # EEGChannelNet
    parser.add_argument('--in-channels', type=int, default=1)
    parser.add_argument('--input-width', type=int, default=276)
    parser.add_argument('--input-height', type=int, default=63)
    parser.add_argument('--num-residual-blocks', type=int, default=3)
    # RNN
    parser.add_argument('--output-size', type=int, default=171)
    ...
    # input_size, output_size as before
    ...
    parser.add_argument('--force-causal-masking', action='store_true')
    parser.add_argument('--use-pretrained-weigths', action='store_true')
    ...
    parser.add_argument('--device', type=str, choices=['cuda:0', 'cuda:1', 'cuda:3', 'cuda:2', 'cpu'], default='cuda:0')
...
def main(args):
    ...
    if args.sweep:
        # Load sweep config
        with open("sweep_config_brainlm.json", "r") as fp:
