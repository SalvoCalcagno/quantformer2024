...
def parse():
    ...
    parser.add_argument('--trace_format', type=str, default='corrected_fluorescence_dff', help='dff or corrected_fluorescence')
...
def main(args):
    ...
    if args.sweep:
        # Load sweep config
        with open("sweep_config_quantformer.json", "r") as fp:
    ...
    def train_func():
        ...
        if args.sweep:
            ...
            #args.exp_name = f"{args.model}_{args.container_id}_{args.stimulus}_lr{args.lr}"
            args.exp_name = f"{args.model}_{args.stimulus}_mr{args.mask_ratio}_lr{args.lr}_K{args.K}_D{args.d_model}_L{args.nlayers}_H{args.nhead}"
            if args.tag is not None:
                args.exp_name += f"_{args.tag}"
