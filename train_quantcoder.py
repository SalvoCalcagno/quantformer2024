...
def parse():
    ...
    parser.add_argument('--trace_format', type=str, default='dff', help='corrected_fluorescence_dff')
    ...
    parser.add_argument('--quick-load', action='store_true')
    ...
    # Model options
    parser.add_argument('--model', default='quantcoder')
    ...
    parser.add_argument('--ablate-quantizer', action='store_true')
    parser.add_argument('--random-init', action='store_true')
    parser.add_argument('--use-neuron-embedding', action='store_true')
    ...
    parser.add_argument('--finetuning', type=str, default='e2e', choices=['e2e', 'linear', 'random_init']) 
    ...
    args.exp_name = f"{args.model}_lr{args.lr}_K{args.K}_D{args.d_model}_L{args.nlayers}_H{args.nhead}"
...
def get_quantizer(stimulus):
    
    # cross stimulus validation
    quantizers = [
        "DG_QUANTIZER_EXPERIMENT_PATH",
        "SG_QUANTIZER_EXPERIMENT_PATH",
        "NS_QUANTIZER_EXPERIMENT_PATH",
        "LSN_QUANTIZER_EXPERIMENT_PATH",    
    ]
    
    for quantizer in quantizers:
        if stimulus in quantizer:
            print(f"Using quantizer {quantizer}")
            return quantizer
...
def main(args):
    ...
    def train_func():
        ...
        # For cross subject validation or single stimulus
        #args.quantizer = get_quantizer(args.stimulus)
        datasets, args = get_datasets(args)
        args.exp_name = f"{args.model}_{args.container_id}_lr{args.lr}_K{args.K}_D{args.d_model}_L{args.nlayers}_H{args.nhead}"
        if args.tag is not None:
            args.exp_name += f"_{args.tag}"
