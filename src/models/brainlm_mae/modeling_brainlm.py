...
class BrainLMEmbeddings(ViTMAEEmbeddings):
    ...
    def __init__(self, config, cls=False):
    ...
    def forward(self, signal_vectors, xyz_vectors, noise):
        ...
        if not self.cls:
            embeddings, mask, ids_restore = self.last_timepoints_masking(x)
            embeddings = torch.flatten(embeddings, start_dim=1, end_dim=2)  # --> [batch, seq, hidden_size]    
            ...
            # append cls token
            cls_tokens = self.cls_token.expand(embeddings.shape[0], embeddings.shape[1], -1, -1)
            embeddings = torch.cat((cls_tokens, embeddings), dim=2)  # CLS token idx 0
            #embeddings = torch.flatten(embeddings, start_dim=1, end_dim=2)  # --> [batch, seq, hidden_size]
    ...
    def last_timepoints_masking(self, sequence):
        ...
        return sequence_unmasked, mask, None
...
class BrainLMForResponseClassification(ViTMAEModel):
    def __init__(self, config):
        super().__init__(config)
        # Overwrite self.embeddings class of superclass ViTMAEModel, rest of attributes are same
        self.embeddings = BrainLMEmbeddings(config, cls=True)  # Embedding logic for fMRI data
        self.encoder = BrainLMEncoder(config)
        self.stim_embedding = nn.Linear(config.stim_dim, config.hidden_size, bias=True)
        self.classifier = nn.Linear(2*config.hidden_size, 2, bias=True)
    ...
    def forward(
        ...
        embedding_output, _, _ = self.embeddings(
            signal_vectors, xyz_vectors, noise
        )
        
        #print(f"embedding_output.shape: {embedding_output.shape}")
        bs, chans, n_patches, patch_len = embedding_output.shape
        embedding_output = torch.flatten(embedding_output, start_dim=1, end_dim=2)  # --> [batch, seq, hidden_size]
        ...
        # restore sequence_output to original shape
        sequence_output = torch.reshape(sequence_output, (bs, chans, n_patches, patch_len))
        ...
        stim = stim.unsqueeze(1).repeat(1, cls_token.shape[1], 1)
        cls_token = torch.cat([cls_token, stim], dim=-1)
...
class BrainLMDecoder(ViTMAEDecoder):
    ...
    def __init__(self, config, num_patches):
        ...
        self.num_last_timepoints_masked = config.num_last_timepoints_masked
        self.num_masked_pacthes = self.num_brain_voxels * (config.num_last_timepoints_masked // config.timepoint_patching_size)
        self.num_masked_patches_per_voxel = config.num_last_timepoints_masked // config.timepoint_patching_size
        ...
        # Final Projection Head (without stim)
        ...
        # Final Projection Head (with stim)
        self.linear_head_with_stim = nn.Sequential(
            nn.Linear(
                in_features=config.decoder_hidden_size*2,
    ...
    def forward(
        ...
        #print(f"flatten_seq_len: {flatten_seq_len}")
        #num_mask_tokens = ids_restore.shape[1] - flatten_seq_len
        #num_mask_tokens = int(flatten_seq_len * self.mask_ratio)
        num_mask_tokens = self.num_masked_pacthes
        num_tokens_per_voxel = self.num_masked_patches_per_voxel

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(batch_size, num_mask_tokens // num_tokens_per_voxel, num_tokens_per_voxel, 1)
        ...
        num_patch_tokens = x.shape[1] // self.num_brain_voxels

        x_ = torch.reshape(
            x[:, 1:, :], shape=(batch_size, self.num_brain_voxels, num_patch_tokens, hidden_dim)
        )  # --> [batch_size, num_voxels, unmasked_timepoints_per_voxel, hidden_size]
        #mask_tokens = mask_tokens.unsqueeze(2)
        x_ = torch.cat([x_, mask_tokens], dim=2)  # --> [batch_size, num_voxels, num_patch_tokens, hidden_size]

        # Add spatial encoding and temporal encoding for decoder
        num_patch_tokens = x_.shape[2]
        ...
        if stim is not None:
            stim = self.stim_embedding(stim)
            stim = stim.unsqueeze(1).repeat(1, hidden_states.shape[1], 1)
            hidden_states = torch.cat([hidden_states, stim], dim=-1)
            logits = self.linear_head_with_stim(hidden_states)
        else:
...
class BrainLMForPretraining(ViTMAEForPreTraining):
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(latent, xyz_vectors, ids_restore, stim=stim)
        logits = (
